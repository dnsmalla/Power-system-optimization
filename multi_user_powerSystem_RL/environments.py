...
class Environment:
    ...
    def step(self, action_n, obs_n, steps):
        ...
        if (self.steps >= self.config["agent1"][3] and self.agent1_act.count(1) < self.config["agent1"][1]) or (
                self.steps >= self.config["agent2"][3] and self.agent1_act.count(1) < self.config["agent2"][1]) or (
                self.steps >= self.config["agent3"][3] and self.agent1_act.count(1) < self.config["agent3"][1]):
        ...
        for i in range(len(obs_n)):
            operation_time[i] = obs_n[i][1] - self.operate[i]
            terminal[i] -= self.operate[i]
            if operation_time[i] == 0:
                break
        # next_obs_n = [[((self.steps + 1) * terminal[i])/24, operation_time[i],self.operate[0],self.operate[1],self.operate[2]] for i in range(self.n)]
        next_obs_n = [[((self.steps + 1) * terminal[i]), operation_time[i], self.supply] for i in range(self.n)]
